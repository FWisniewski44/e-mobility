...
;; anfrage für individuals dass der anteil von 2,6% die procedure von "adopted" (siehe unten) haben soll; Schwerpunkt auf Innovation-Leaders und Eco-oriented Leaders
...
;; funktioniert nicht, wegen turtle zugehörigkeit, Lösung: neighbor count muss pro tick sowieso geupdated werden --> neigbour_count am anfang der go funktion
;;to count_neighbor
;;  ask individual with [adopt? = false] [set adopted_neighbors count (link-neighbors with [adopt? = true])]
...
;;to spread-ev ;; könnte schon funktionieren: methode, die von link-neighbors aus auf die bisher "not-adapted turtles" einwirken soll
  ;;ask individual with [adopt? = false]
    ;;[ if adopted_neighbors > 0 [adopted]]
;;end
...
to setup
  ...
  ask individual with [adopt? = false] [countn]
  ask individual [set belief-e (infrastructure * pcs)]
  reset-ticks
...
to spread-ev ;; könnte schon funktionieren: methode, die von link-neighbors aus auf die bisher "not-adapted turtles" einwirken soll
  ask individual [set utility-u (subsidies * adopted_neighbors)]
  ask individual [set behavior-bi (aoev + utility-u + belief-e)]
  ask individual with [adopt? = false]
